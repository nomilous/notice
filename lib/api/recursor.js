...
module.exports.recursor = function(local, type) {
  return testable = function(_arg, request, response, statusCode) {
    var authenticEntity, deeper, notifier, object, path, query, searialized, uuid;
    query = _arg[0], uuid = _arg[1], deeper = _arg[2], authenticEntity = _arg[3];
    if (statusCode == null) {
      statusCode = 200;
    }
    ...
    return recurse(authenticEntity, request, object, path, {}, function(error, result) {
      var body;
      if (deeper != null) {
...
recurse = function(authenticEntity, request, object, path, result, callback) {
  var key, next;
  ...
  for (key in object) {
    ...
    switch (typeof object[key]) {
      case 'object':
        result[key] = {};
        recurse(authenticEntity, request, object[key], path, result[key]);
        break;
      case 'number':
      case 'string':
