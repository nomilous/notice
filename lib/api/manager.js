...
module.exports.manager = function(config) {
  ...
  testable = local = {
    ...
    middleware: function(action, hub, slot, contentType, body, response, statusCode) {
      ...
      */
      ...
      if ((typeof mware !== "undefined" && mware !== null) && (mware.title === 'first' || mware.title === 'last')) {
        return local.badRequest(response, {
          error: {
            type: errorType || 'Error',
  ...
  local.routes = {
    ...
    '/hubs/:uuid:': {
      ...
      handler: function(_arg, request, response, statusCode) {
        var notifier, opts, uuid;
        opts = _arg[0], uuid = _arg[1];
        if (statusCode == null) {
          statusCode = 200;
        }
        if (request.method !== 'GET') {
    ...
    '/hubs/:uuid:/middlewares': {
      ...
      handler: function(_arg, _arg1, response, statusCode) {
        var body, headers, hubuuid, method, nothing, notifier, opts;
        opts = _arg[0], hubuuid = _arg[1], nothing = _arg[2];
    ...
    '/hubs/:uuid:/middlewares/:slot:': {
      ...
      accepts: ['text/javascript', 'text/coffeescript'],
      handler: function(_arg, _arg1, response, statusCode) {
        var body, headers, hubuuid, method, middlewares, notifier, opts, slot;
        opts = _arg[0], hubuuid = _arg[1], slot = _arg[2];
        method = _arg1.method, headers = _arg1.headers, body = _arg1.body;
        if (statusCode == null) {
          statusCode = 200;
        }
    ...
    '/hubs/:uuid:/middlewares/:slot:/fn': {
      ...
      methods: ['GET'],
      handler: function(_arg, request, response, statusCode) {
        var fnString, middlewares, notifier, opts, slot, uuid;
        opts = _arg[0], uuid = _arg[1], slot = _arg[2];
        if (statusCode == null) {
          statusCode = 200;
        }
        if (request.method !== 'GET') {
    ...
    '/hubs/:uuid:/middlewares/:slot:/disable': {
      ...
      methods: ['GET'],
      handler: function(_arg, request, response, statusCode) {
        var middlewares, notifier, opts, slot, uuid;
        opts = _arg[0], uuid = _arg[1], slot = _arg[2];
        if (statusCode == null) {
          statusCode = 200;
        }
  ...
  _ref = start(opts, local.requestHandler = authenticated(function(authenticEntity, request, response) {
    ...
    return request.on('end', function() {
      var action, base, deeper, match, nested, path, query, slot, uuid, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
      if (error) {
        return;
      }
      request.body = body;
      path = request.url;
      try {
        _ref = path.split('?', 2), path = _ref[0], query = _ref[1];
      } catch (_error) {}
      opts = {
      ...
      try {
        ...
        try {
          if (_ref4 = path.match(/\/hubs\/(.*)\/stats\/(.*)/), match = _ref4[0], uuid = _ref4[1], deeper = _ref4[2], _ref4) {
            return local.routes["/hubs/:uuid:/stats/**/*"].handler([opts, uuid, deeper], request, response);
          }
        } catch (_error) {}
        try {
          if (_ref5 = path.match(/\/hubs\/(.*)\/cache\/(.*)/), match = _ref5[0], uuid = _ref5[1], deeper = _ref5[2], _ref5) {
            return local.routes["/hubs/:uuid:/cache/**/*"].handler([opts, uuid, deeper], request, response);
          }
        } catch (_error) {}
        try {
          if (_ref6 = path.match(/\/hubs\/(.*)\/tools\/(.*)/), match = _ref6[0], uuid = _ref6[1], deeper = _ref6[2], _ref6) {
      ...
      try {
        _ref7 = path.match(/\/(.*)\/(.*)/), match = _ref7[0], base = _ref7[1], uuid = _ref7[2];
        return local.routes["/" + base + "/:uuid:"].handler([opts, uuid], request, response);
      } catch (_error) {}
      try {
        _ref8 = path.match(/\/(.*)/), match = _ref8[0], base = _ref8[1];
