...
module.exports.manager = function(config) {
  ...
  testable = local = {
    ...
    middleware: function(action, hub, slot, request, response, statusCode) {
      return local[action + 'Middleware'](hub, slot, request, response, statusCode);
    },
    insertMiddleware: function(hub, slot, middleware, response, statusCode) {
      return local.respond({
        insert: 'new'
      }, 200, response);
    },
    upsertMiddleware: function(hub, slot, middleware, response, statusCode) {
      console.log({
        update: slot
      });
      return local.respond({
