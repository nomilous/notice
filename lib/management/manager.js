...
module.exports.manager = function(config) {
  var address, api, authenticated, listen, local, opts, port, recurse, recursed, server, transport, _ref;
  if (config == null) {
    config = {};
  }
  try {
  ...
  recursed = function(type, resultHandler) {
    return function(_arg, request, response, statusCode) {
      ...
      return resultHandler(recurse(notifier.serialize(2)[type]), request, response, statusCode);
    };
  };
  testable = local = {
    hubContext: void 0,
