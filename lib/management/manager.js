...
module.exports.manager = function(config) {
  var address, api, authenticated, listen, local, opts, port, recurse, recurses, server, transport, _ref;
  if (config == null) {
    config = {};
  }
  try {
  ...
  recurse = function(object, pathArray, accum) {
    var key, nested, next;
    if (accum == null) {
      accum = {};
    }
    ...
    for (key in object) {
      ...
      if (typeof nested === 'function' && (nested.$$notable != null)) {
  ...
  recurses = function(type, resultHandler) {
    return function(_arg, request, response, statusCode) {
      var deeper, notifier, uuid;
      ...
      return resultHandler(recurse(notifier.serialize(2)[type], deeper.split('/')), request, response, statusCode);
    };
  };
  testable = local = {
    hubContext: void 0,
    ...
    routes: {
      ...
      '/v1/hubs/:uuid:/tools': {
        ...
        methods: ['GET'],
        handler: recurses('tools', function(result, request, response, statusCode) {
          if (statusCode == null) {
            statusCode = 200;
          }
