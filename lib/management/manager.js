...
module.exports.manager = function(config) {
    ...
    routes: {
      ...
      '/v1/hubs/:uuid:/tools': {
        ...
        handler: function(_arg, request, response, statusCode) {
          ...
          notifier = local.hubContext.uuids[uuid];
          return local.respond(notifier.serialize(2).tools, statusCode, response);
        }
      },
      '/v1/hubs/:uuid:/cache/**/*': {
  ...
  _ref = start(opts, local.requestHandler = authenticated(function(request, response) {
    var action, base, deeper, match, nested, path, title, uuid, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
    path = request.url;
    if (path === '/about' || path === '/') {
    ...
    try {
      _ref4 = path.match(/(.*)\/(.*)\/(.*)/), match = _ref4[0], version = _ref4[1], base = _ref4[2], uuid = _ref4[3];
      return local.routes["" + version + "/" + base + "/:uuid:"].handler([uuid], request, response);
    } catch (_error) {}
    try {
      _ref5 = path.match(/(.*)\/(.*)/), match = _ref5[0], version = _ref5[1], base = _ref5[2];
