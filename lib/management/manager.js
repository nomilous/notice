...
module.exports.manager = function(config) {
  ...
  testable = local = {
    ...
    routes: {
      ...
      '/v1/hubs/:uuid:/middlewares/:id:': {
        ...
        handler: function(_arg, request, response, statusCode) {
          var id, middleware, middlewares, notifier, uuid, _i, _len;
          uuid = _arg[0], id = _arg[1];
          if (statusCode == null) {
            statusCode = 200;
          }
  ...
  _ref = start(opts, local.requestHandler = authenticated(function(request, response) {
    var error, id, match, matched, nested, path, uuid, _ref, _ref1, _ref2;
    path = request.url;
    if (path.slice(-1) === '/') {
      path = path.slice(0, -1);
    }
    try {
      ...
      if (_ref1 = uuid.match(/(.*?)\/(.*)/), match = _ref1[0], uuid = _ref1[1], nested = _ref1[2], _ref1) {
        ...
        if (_ref2 = nested.match(/(.*?)\/(.*)/), match = _ref2[0], nested = _ref2[1], id = _ref2[2], _ref2) {
