  ...
  testable = local = {
    ...
    create: deferred(function(_arg, clientName, opts, callback) {
      ...
      socket.on('disconnect', function() {
        if (!((client.connection != null) && client.connection.state === 'accepted')) {
      ...
      return socket.on('error', function(error) {
        ...
        if (opts.connect.retryWait == null) {
          delete local.clients[clientName];
          setTimeout((function() {
            reject(error);
            if (typeof callback === 'function') {
        ...
        (_base = client.connection).retryStartedAt || (_base.retryStartedAt = Date.now());
        if ((_base1 = client.connection).retryCount == null) {
          _base1.retryCount = -1;
        }
