  ...
  Testable = Handler = {
    create: function(hubName, hubNotifier, hubContext, opts) {
      ...
      hubNotifier.use({
        ...
        first: true
      }, function(next, raw, traversal) {
        var assign, capsule, client, hidden, id, payload, property, tected, uuid;
        if (!(id = raw._socket_id)) {
          next();
        }
        try {
        ...
        try {
          uuid = raw.control.uuid;
        } catch (_error) {}
        try {
          tected = raw.control["protected"];
        } catch (_error) {}
        try {
          hidden = raw.control.hidden;
        } catch (_error) {}
        try {
          payload = raw.payload;
        } catch (_error) {}
