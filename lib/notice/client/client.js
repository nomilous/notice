...
var Connector, PROTOCOL_VERSION, alreadyDefined, connectRejected, deferred, disconnected, hostname, notifier, reservedCapsule, terminal, testable, ticker, undefinedArg, _ref;
  ...
  testable = local = {
    ...
    create: deferred(function(_arg, title, opts, callback) {
      ...
      client.use({
        ...
        var control, header, wait;
        if (capsule.$tick != null) {
          return;
        }
        /* grep PROTOCOL1 encode*/

        wait = capsule.$wait ? 1 : 0;
      ...
      socket.on('ack', function(control) {
        var capsule, next, uuid, wait, _ref1;
        try {
          uuid = control.uuid;
          _ref1 = transit[uuid], next = _ref1.next, wait = _ref1.wait, capsule = _ref1.capsule;
          if (!wait) {
            delete transit[uuid];
            return next();
          }
